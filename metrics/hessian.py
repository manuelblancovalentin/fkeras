...
# [@manuelbv]: Added tqdm to keep track of progress
...
class HessianMetrics:
    ...
    def top_k_eigenvalues(self, k=1, max_iter=100, tolerance=1e-3, rank_BN=True, prefix = None, **kwargs):
        """
        Compute the top k eigenvalues and eigenvectors of the Hessian using the
        power iteration method. The eigenvalues are sorted in descending order.
        k: number of eigenvalues to compute
        ...
        with tqdm(total=k*max_iter, **kwargs) as pbar:
            for iter in range(k):
                # Set description
                pbar.set_description(f"{prefix} - Eigenvalue {iter+1}/{k}")
                eigenvalue = None
                # Initialize the eigenvector
                v = [np.random.uniform(size=p.shape) for p in params]
                v = [tf.convert_to_tensor(vi, dtype=tf.dtypes.float32) for vi in v]
                ...
                for j in range(max_iter):
                    ...
                    if eigenvalue == None:
                        ...
                        rel_err = abs(tmp_eigenvalue - eigenvalue) / (abs(eigenvalue) + 1e-6)
                        if (
                            rel_err < tolerance
                        ):
                            break
                    ...
                    if j % 10 == 0:
                        pbar.update(10)
                        pbar.set_description(f"{prefix} - Eigenvalue {iter+1}/{k}... Rel. Error: {rel_err:.2e} > {tolerance:.2e}")
                # Make sure we update the pbar to the correct value (i/k)
