...
class HessianMetrics:
    ...
    def get_supported_layer_indices(self):
        ...
        for i in self.layer_indices:
            ly = self.model.layers[i]
            if hasattr(ly, 'layer'):
                ly = ly.layer
            if ly.__class__.__name__ in SUPPORTED_LAYERS:
                #print(f"Layer {ly.__class__.__name__} is supported")
                supported_indices.append(running_idx)
            # else:
            #     print(f"Layer {ly.__class__.__name__} is not supported")
        ...
        for batch_x, batch_y in zip(self.batched_x, self.batched_y):
            with tf.GradientTape() as outer_tape:
                with tf.GradientTape() as inner_tape:
                    # make sure batch_y is same type as batch_x (basically, that's NOT integer)
                    batch_y = tf.cast(batch_y, batch_x.dtype)
                    y_hat = self.model(batch_x, training=True)
                    loss = self.loss_fn(batch_y, y_hat)
    ...
    def top_k_eigenvalues(self, k=1, max_iter=100, tolerance=1e-3, rank_BN=True, prefix=""):
        """
        Compute the top k eigenvalues and eigenvectors of the Hessian using the
        power iteration method. The eigenvalues are sorted in descending order.
        k: number of eigenvalues to compute
        ...
        with tqdm(total=k*max_iter, desc=f"{prefix}Computing eigenvalue ") as pbar:
            for kiter in range(k):
                eigenvalue = None
                # Update description
                pbar.set_description(f"{prefix}Computing eigenvalue {kiter}")
                # Initialize the eigenvector
                v = [np.random.uniform(size=p.shape) for p in params]
                v = [tf.convert_to_tensor(vi, dtype=tf.dtypes.float32) for vi in v]
                # Normalize the eigenvector
                ...
                for j in range(max_iter):
                    ...
                    if eigenvalue == None:
                        ...
                        if (
                            ...
                            # Update pbar with remaining iterations
